...
def train_mfusion(model, device, train_loader, test_loader, num_epochs, optimizer, criterion, save_root, lr):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (inputs, targets) in enumerate(train_loader):
            ...
            outputs, fc_inputs = model(inputs)
            loss = criterion(outputs, targets)
            pred = outputs.argmax(dim=1, keepdim=True)
            correct += pred.eq(targets.view_as(pred)).sum().item()

            fc1_input, fc2_input, fc3_input = fc_inputs
            ...
            fc2_weight_fixed = fc2_weight[:num_class, :last_channel]
            grad_output_est = error.mm(fc2_weight_fixed.view(-1, np.prod(fc2_weight_fixed.shape[1:])))
            ...
            fc2_delta_w = -fc2_lr * grad_output_est * fc2_input_scale * grad_output_est_scale
            model.fc2.weight.data = model.fc2.weight.data + fc2_delta_w

            fc1_weight = model.fc1.weight.data.clone()
            fc1_weight = w_quantizer_noise(fc1_weight)
            fc1_weight_fixed = fc1_weight[:num_class, :last_channel]
            grad_output_est = error.mm(fc1_weight_fixed.view(-1, np.prod(fc1_weight_fixed.shape[1:])))
            grad_output_est, grad_output_est_s = output_quantizer(grad_output_est)
            grad_output_est = grad_output_est * grad_output_est_s
            ...
            grad_output_est = torch.matmul(grad_output_est.T, fc1_input) / outputs.shape[0]
            grad_output_est, grad_output_est_scale = error_quantizer(grad_output_est)
            fc1_delta_w = -fc1_lr * grad_output_est * fc1_input_scale * grad_output_est_scale
...
if __name__ == '__main__':
    ...
    data_dir = r'datas'
    save_root = 'checkpoint/quant_noise_incre_mfusion_qn'
    ...
    train_mfusion(model, device, train_loader, test_loader, num_epochs, optimizer, criterion, save_root, lr=lr)
