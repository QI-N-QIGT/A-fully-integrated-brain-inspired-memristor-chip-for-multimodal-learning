...
class MultiModalDataset_noise_speech(Dataset):

    def __init__(self, root: str, split: str, miss_mode='', noise_ratio=None):
        self.root = root
        self.split = split
        ...
        if noise_ratio is not None:
            ori_data = np.load(os.path.join(self.root, 'noise_%.1f_speech_datas.npy' % noise_ratio), allow_pickle=True)
        else:
            ori_data = np.load(os.path.join(self.root, 'noise_speech_datas.npy'), allow_pickle=True)
        index_path = self.train_index_path if self.split == 'train' else self.test_index_path
        index = np.load(index_path)
        if split == 'train':
            self.samples = ori_data[index][::15]
        else:
            self.samples = ori_data[index][::4]
...
if __name__ == '__main__':
    ...
    offset_row_begin = 8
    sa_code_map = 3
    save_root = 'checkpoint/quant_noise_incre_dfa_miss'
    save_root += '_' + miss + '_speech_noise_%.1f' % noise_ratio
    train_set = MultiModalDataset_noise_speech(root=data_dir, split='train', miss_mode=miss, noise_ratio=noise_ratio)
    test_set = MultiModalDataset_noise_speech(root=data_dir, split='test', miss_mode=miss, noise_ratio=noise_ratio)
