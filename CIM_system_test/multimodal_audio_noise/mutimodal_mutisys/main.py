        ...
        if g_radar is not None and g_speech is not None and (g_img is not None):
            with lock1:
                with lock2:
                    with lock3:
                        print()
                        (speed_x, speed_y) = controller.get_mutimodal_speed(g_radar, g_speech, g_img, method='a111', chip_id=7)
...
class Evaluator_Noise_Speech:
    ...
    def __init__(self, modal, simulated_data_root, chip_num, timestamp, offset_row_begin=8, step=5):
        ...
        model.load_state_dict(torch.load(os.path.join('simulated_data/noise_speech_best_model.pth'), device))
        model.to(device)
        model.eval()
...
class Controller:
    ...
    def __init__(self):
        ...
        step = 10
        debug_img_number = img_number // step
        self.mutimodal = Evaluator_Noise_Speech(modal, simulated_data_root, chip_num, timestamp, step=step)
        print()
        print()
        print()
        print()
