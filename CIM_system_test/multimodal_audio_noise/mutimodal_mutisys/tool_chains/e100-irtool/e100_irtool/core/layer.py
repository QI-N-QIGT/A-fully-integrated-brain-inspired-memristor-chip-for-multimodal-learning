...
class BaseLayer(Jsonable, RegBase, metaclass=LayerReg):
    ...
    def __init__(self, *, inputs=None, outputs=None, weights=None, datadef=DataDef, **kwargs):
...
class OpLayer(BaseLayer):
    ...
    def validate_graph(self):
        ...
        if self.op.num_inputs is None:
            ...
            assert n == self.op.num_inputs, f'invalid {n} inputs, expects {self.op.num_inputs}'
...
class LayerTree(Jsonable):
            ...
            with self.with_layer(name) as layer:
                ...
                try:
                    ...
                    if deep and isinstance(layer, LayerTree):
                        yield from layer.iter_layers(deep=deep, sorted=sorted, _ns=_ns)
    ...
    def is_simple_graph(self):
        if not self.layers:
            return True
        return not self.layers or all((not layer.is_layer_tree() or layer.is_simple_graph() for layer in self.layers.values()))
    ...
    def validate_graph(self):
        ...
        for (key, layer) in self.iter_layers(deep=False):
            ...
            if layer.type == 'reuse':
                assert layer.layer in self.layers, f'invalid reuse layer {layer.layer}'
