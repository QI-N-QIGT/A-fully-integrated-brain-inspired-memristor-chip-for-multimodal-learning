import re
RE_NAME = re.compile('^[a-zA-Z][a-zA-Z0-9]*(?:[_\\-][a-zA-Z0-9]+)*(?:\\:\\d+)?$')
...
def make_ref(*names):
    names = [name if isinstance(name, str) else name[0] if name[1] is None else f'{name[0]}:{name[1]}' for name in names]
