...
from ..runtime import load_runtime, enum_runtimes
...
class BaseCmd(metaclass=CmdReg):
    ...
    @classmethod
    def arg_runtime(cls, argp):
        argp.add_argument('-rt', '--runtime', choices=enum_runtimes(), default=os.environ.get('E100_RUNTIME', 'numpy'), help='select runtime, env E100_RUNTIME')
    ...
    def arg_input_file(cls, argp, *, nargs=None):
        if nargs is None:
            ...
            argp.add_argument('input_file', nargs=nargs, type=Path, help='input files')
    ...
    def arg_output_file(cls, argp, nargs=None):
        if nargs is None:
            ...
            argp.add_argument('output_file', nargs=nargs, type=Path, help='output file')

    @classmethod
    def arg_channel_last(cls, argp):
        g = argp.add_mutually_exclusive_group()
        g.add_argument('-cl', '--channel-is-last', action='store_true', help='channel following dims')
        g.add_argument('-cf', '--channel-is-first', action='store_true', help='channel before dims')

    @classmethod
    def arg_dims(cls, argp):
        argp.add_argument('-d', '--dims', help='":" seperated shapes without batch and channel, eg: "2,3:3,4"')
    ...
    def parse_dims(self, dims=None):
        ...
        if dims is not None:
            try:
                return tuple((tuple((int(d) for d in dim.split(','))) for dim in dims.split(':')))
            except ValueError:
