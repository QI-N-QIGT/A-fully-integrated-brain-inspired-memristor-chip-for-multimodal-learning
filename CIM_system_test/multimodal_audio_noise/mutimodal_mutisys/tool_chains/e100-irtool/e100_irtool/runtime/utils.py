...
def to_auto_pad(dims, kernel, stride, padding, dilation):
    ...
    if all((p[i] == q[i] // 2 and p[i] + p[ndim + i] == q[i] for i in range(ndim))):
        return SAME_PAD
    assert False, f'not VALID or SAME padding: input {x}, kernel {k}, stride {s}, padding {padding}, dilation {dilation}'
...
def pool_shapes(dims, kernel, stride, padding, dilation, auto_pad, ceil_mode=False):
    ndim = len(dims)
    x = dims
    k = to_int_tuple(kernel, ndim=ndim)
    s = to_int_tuple(stride, ndim=ndim)
    ...
    if padding is None:
        ...
        p = to_int_tuple(padding, ndim=ndim * 2)
    if ceil_mode:
        os = [(x[i] + p[i] + p[ndim + i] - dk[i] + s[i] - 1) // s[i] + 1 for i in range(ndim)]
        for i in range(ndim):
            if (os[i] - 1) * s[i] >= x[i] + p[i]:
                os[i] -= 1
        os = tuple(os)
    else:
        os = tuple(((x[i] + p[i] + p[ndim + i] - dk[i]) // s[i] + 1 for i in range(ndim)))
...
def conv_t_shapes(dims, kernel, stride, padding, dilation, output_padding, auto_pad):
    ndim = len(dims)
    x = dims
    k = to_int_tuple(kernel, ndim=ndim)
    s = to_int_tuple(stride, ndim=ndim)
    ...
    assert all((i >= 0 for i in dp)), f'invalid padding {padding} for kernel {kernel} dilation {dilation}'
    di = tuple(((x[i] - 1) * s[i] + 1 + dp[i] + dp[ndim + i] + a[i] for i in range(ndim)))
    os = tuple(((x[i] - 1) * s[i] - p[i] - p[ndim + i] + dk[i] + a[i] for i in range(ndim)))
...
def resize_size_scale(dims, size, scale):
    ...
    if size is not None:
        ...
        if isinstance(size, int):
            ...
            assert len(size) == ndim, f'invalid resize {dims} to size {size}'
            size = tuple(size)
        ...
        if isinstance(scale, (float, int)):
            ...
            assert len(scale) == ndim, f'invalid resize {dims} with scale {scale}'
            scale = tuple(scale)
...
def concat_axis(rank, channel_last, axis, with_batch, channel_pos):
    ...
    assert channel_pos in ('first', 'last', 'ignore'), f'invalid channel_pos {channel_pos!r}'
...
def split_to_secs(split, size):
    ...
    if isinstance(split, int):
        ...
        assert sum(split) == size, f'invalid split {size} to {split}'
        s = split
    return tuple(s)