...
class ShapeInferer:
    all_ops = {'const': {'constant'}, 'unary': {'identity', 'sign', 'abs', 'neg', 'ceil', 'floor', 'exp', 'log', 'sqrt', 'sin', 'cos', 'tan', 'asin', 'acos', 'atan', 'sinh', 'cosh', 'tanh', 'asinh', 'acosh', 'atanh', 'logical_not', 'bitwise_not', 'relu', 'leaky_relu', 'prelu', 'selu', 'celu', 'elu', 'softmax', 'log_softmax', 'sigmoid', 'hard_sigmoid', 'softplus', 'softsign', 'silu'}, 'binary': {'add', 'sub', 'mul', 'div', 'mod', 'pow', 'logical_and', 'logical_or', 'logical_xor', 'bitwise_and', 'bitwise_or', 'bitwise_xor', 'equal', 'less', 'less_or_equal', 'greater', 'greater_or_equal'}, 'trans': {'concat', 'reshape', 'flatten', 'transpose', 'pad', 'split'}, 'norm': {'batch_norm', 'batch_norm1d', 'batch_norm2d', 'batch_norm3d', 'instance_norm1d', 'instance_norm2d', 'instance_norm3d'}, 'pool': {'avgpool1d', 'avgpool2d', 'avgpool3d', 'avg_pool1d', 'avg_pool2d', 'avg_pool3d', 'maxpool1d', 'maxpool2d', 'maxpool3d', 'max_pool1d', 'max_pool2d', 'max_pool3d'}, 'global_pool': {'global_avg_pool1d', 'global_avg_pool2d', 'global_avg_pool3d', 'global_max_pool1d', 'global_max_pool2d', 'global_max_pool3d'}, 'matmul': {'matmul', 'linear', 'fc'}, 'conv': {'conv1d', 'conv2d', 'conv3d'}, 'conv_transpose': {'conv_transpose1d', 'conv_transpose2d', 'conv_transpose3d'}, 'resize': {'resize'}, 'others': {'reducemean'}}
    ...
    def fn_concat(self, op, *x, channel_last, **kwargs):
        ...
        axis = concat_axis(rank, channel_last, op.axis, op.with_batch, op.channel_pos)
        for s in x[1:]:
            assert s[:axis] == x0[:axis] and s[axis + 1:] == x0[axis + 1:], f"can't concat shape {x0} with {s}"
    ...
    def fn_reshape(self, op, x, **kwargs):
        ...
        assert math.prod(x) == math.prod(shape), f"can't reshape {x} to {shape}"
        return tuple(shape)
    ...
    def fn_transpose(self, op, x, **kwargs):
        perm = tuple(op.perm)
        if op.with_batch:
            assert perm[0] == 0, "axis 0 (batch) can't transpose to other axis"
    ...
    def fn_pad(self, op, x, *, channel_last, **kwargs):
        ...
        ndim = len(xd)
        p = op.pads
        assert len(p) == ndim * 2, f'invalid pads {p} rank != {ndim}*2'
    ...
    def fn_pool(self, op, x, *, channel_last, **kwargs):
        ndim = op.ndim
        assert len(x) == ndim + 1, f'invalid input rank {len(x)} != ndim({ndim}) + 1'
        if channel_last:
            (*xd, ci) = x
        else:
            (ci, *xd) = x
        (os, _) = pool_shapes(xd, op.kernel, op.stride, op.padding, op.dilation, op.auto_pad, op.ceil_mode)
        return (*os, ci) if channel_last else (ci, *os)
    ...
    def fn_conv(self, op, x, *, channel_last, **kwargs):
        if channel_last:
            ...
            (ci, *xd) = x
        assert ci == op.in_channel, f'invalid input {x} channel {ci} != {op.in_channel}'
        co = op.out_channel
        (os, _) = conv_shapes(xd, op.kernel, op.stride, op.padding, op.dilation, op.auto_pad)
    ...
    def fn_conv_transpose(self, op, x, *, channel_last, **kwargs):
        ...
        assert ci == op.in_channel, f'invalid input {x} channel {ci} != {op.in_channel}'
        co = op.out_channel
        (os, _) = conv_t_shapes(xd, op.kernel, op.stride, op.padding, op.dilation, op.output_padding, op.auto_pad)
        return (*os, co) if channel_last else (co, *os)
