        ...
        if i.output[0] not in outputs:
            if i.output[0] not in value_info.keys() or not value_info_has_shape(value_info[i.output[0]]):
                if is_input_ready(value_info, i.input, inferenced_output_name_list, initializer_dict, constant_value_dict):
                    ...
                    node_output_value_info = helper.make_tensor_value_info(i.output[0], node_output_type, node_output_shape)
            ...
            node_list.pop(0)
    graph = helper.make_graph(model.graph.node, model.graph.name, model.graph.input, model.graph.output, model.graph.initializer, doc_string=None, value_info=value_info.values())
...
def load_onnx_model(model, label_file=None, output_npu_model_filename=None, store_intermediate_model=False, fix_layer_name=False):
    ...
    if node_output != []:
        ...
        for i in range(len(model.graph.input)):
            ...
            if len(input_dim) == 4:
                ...
                raise ValueError(f'暂不支持输入维度{input_dim}')
        ...
        for n in range(len(node_output)):
            new_initializer_dict.append(make_tensor(node_output[n], outputs_val[n + 1]))
        ...
        npu_compatible_graph = helper.make_graph(node_list, model.graph.name, model.graph.input, model.graph.output, new_initializer_dict, doc_string=None, value_info=model.graph.value_info)
    ...
    if fix_layer_name:
        npu_model = fix_node_name(npu_model)
    ...
    for node in model.graph.node:
        if node.op_type == 'Pad':
            ...
            if np.mean(pad_value) == 0 and np.std(pad_value) == 0:
                ...
                outputNode_new = make_node(outputNode.op_type, inputs, outputNode.output, outputNode.name, **attr)
...
def get_model_addr(model_attr):
    ...
    for a in model_attr:
        if a.name in ['group', 'ceil_mode', 'axis']:
            ...
            raise ValueError(f'暂不支持 {a} !!!')
    return attr

def make_node(op_type, inputs, outputs, name=None, domain=None, doc_string=None, **kwargs):
    if name is None:
        name = op_type
    node = helper.make_node(op_type, [i for i in inputs if i is not None], [o for o in outputs if o is not None], name, doc_string, domain, **kwargs)
    return node