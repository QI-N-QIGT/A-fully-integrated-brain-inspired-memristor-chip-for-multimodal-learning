...
from e100_irmapper.device.a111 import MappedLayer
...
class A111NumpyRT(FusedOpNumpyRuntime):
    ...
    def run_ir(self, ir, inputs, weights=None, *, outputs=None, callback=None, logfile={'dump_reg': f'reg_file/dumped_reg_file_{time_record}.txt', 'dump_serial_script': False, 'verbose': True, 'log_file': f'log_file/log_file_{time_record}.txt'}):
                ...
                for (tile_name, tile) in device.devices.items():
                    if tile.kind == 'a111-tile':
                        if tile.info.op_list != None:
                            ...
                            (tileop_config, last_esram_addr) = self.config_tileop(tile_name, tile, layers, batch_num=batch_num, last_esram_addr=last_esram_addr)
        ...
        for (name, layer) in layers.items():
            ...
            if layer.op.op_id in ['conv2d', 'matmul', 'fc', 'linear', 'fused_conv2d', 'fused_fc']:
                if name in a111_layer.keys():
                    if a111_layer[name] not in configured_tile_list:
                        ...
                        y = self.run_tile(a111_layer[name], *x, *tile_layer_info, *tile_device_info, *tileop_info, *rpc, **tile_ats, **logfile)
            ...
            if not isinstance(y, (tuple, list)):
                y = (y,)
            if callback is not None:
                callback(name, layer=layer, inputs=x, weights=tile_wts, attrs=tile_ats, outputs=y)
            data[name] = tuple(y)
    ...
    def run_tile(self, tile_name, *args, **kwargs):
        ...
        if last_op_type == 'FC':
            ...
            output_feature_size = [batch_num, last_layer.outputs[0].channel, last_layer.outputs[0].height, last_layer.outputs[0].width]
        ...
        output = rpc_api.call('a111_run', tile_id, xb_id_list, tile_mode, in_data, output_feature_size, in_addr_esram=in_addr_esram, out_addr_esram=out_addr_esram, first_op_type=first_op_type, last_op_type=last_op_type, last_relu=last_relu, dump_reg=kwargs['dump_reg'], dump_serial_script=kwargs['dump_serial_script'], verbose=kwargs['verbose'], log_file=kwargs['log_file'])
    ...
    def config_tileop(self, tile_name, tile_info, layer_info, batch_num=1, last_esram_addr=[]):
        ...
        pool_en_list = [tile_info.info.pool0_en, tile_info.info.pool1_en, tile_info.info.pool2_en, tile_info.info.pool3_en]
        xb_arr_sel = 0
        if res_num == 1:
            xb_arr_sel = 3
        ...
        output_addr_list = [0, 0, 0, 0]
        out_img_size_list = [[1, 1], [1, 1], [1, 1], [1, 1]]
        xbg_axi_cnt_list = [0, 0, 0, 0]
        in_buf_type_list = [0, 0, 0, 0]
        out_buf_type_list = [0, 0, 0, 0]
        linebuf_addr_offset_list = [0, 0, 0, 0]
        linebuf_width_list = [0, 0, 0, 0]
        relu_th_list = [0, 0, 0, 0]
        act_mode_list = [0, 0, 0, 0]
        shift_list = [0, 0, 0, 0]
        for layer_name in tile_info.info.op_list:
            ...
            if input_len_list[index] <= 12288:
                in_buf_type_list[index] = input_len_list[index] // (2048 + 1)
            elif input_len_list[index] <= 16384:
                in_buf_type_list[index] = 6
            elif input_len_list[index] <= 32768:
            ...
            if layer_.a111_mapping_info.output_tile_buffer_addr != None:
                output_addr_list[index] = 2013265920 + layer_.a111_mapping_info.output_tile_buffer_addr[0]
            else:
                output_addr_list[index] = 1744830464 + tile_info.info.out_esram_addr[0]
            ...
            if xbg_axi_cnt_list[index] <= 12288:
                out_buf_type_list[index] = xbg_axi_cnt_list[index] // (2048 + 1)
            elif xbg_axi_cnt_list[index] <= 16384:
                out_buf_type_list[index] = 6
            elif xbg_axi_cnt_list[index] <= 32768:
        ...
        return (TileOpObj(tile_id=tile_id, xb_id_list=xb_id_list, tile_mode=tile_mode, pool_en_list=pool_en_list, xb_arr_sel=xb_arr_sel, xbg_mode_list=xbg_mode_list, xbg_calc_mode_list=xbg_calc_mode_list, xbg_in_pix_type_list=xbg_in_pix_type_list, xbg_out_pix_type_list=xbg_out_pix_type_list, xbg_kernel_type_list=xbg_kernel_type_list, xbg_toggle_en0_list=xbg_toggle_en0_list, xbg_toggle_bit0_list=xbg_toggle_bit0_list, xbg_tile_buf_en0_list=xbg_tile_buf_en0_list, xbg_tile_cal_en0_list=xbg_tile_cal_en0_list, xbg_fcn_en0_list=xbg_fcn_en0_list, xbg_relu_en_list=xbg_relu_en_list, xb_start_column_list=xb_start_column_list, xb_column_num_list=xb_column_num_list, input_addr_list=input_addr_list, input_len_list=input_len_list, in_img_size_list=in_img_size_list, output_addr_list=output_addr_list, out_img_size_list=out_img_size_list, xbg_axi_cnt_list=xbg_axi_cnt_list, in_buf_type_list=in_buf_type_list, out_buf_type_list=out_buf_type_list, linebuf_addr_offset_list=linebuf_addr_offset_list, linebuf_width_list=linebuf_width_list, relu_th_list=relu_th_list, act_mode_list=act_mode_list, shift_list=shift_list, adc_range_list=adc_range_list, res_in_sel=res_in_sel, res_out_sel=res_out_sel, res_9bit_en=res_9bit_en, pad_en_list=pad_en_list), last_esram_addr)