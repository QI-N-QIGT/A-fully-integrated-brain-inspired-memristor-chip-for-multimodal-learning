...
def shift_clip_for_4bit_input(input, scale, method=1, shift=0, clip_bit=4):
    if method == 1:
        ...
        return ((input / 16.0).frac() * 16.0, scale)
    elif method == 3:
...
def xbar_basic_block_hard_method(method, input, input_scale, weight, weight_scale, output_scale, in_quant_low_bit, input_scale_hard, weight_scale_hard, output_scale_hard):
...
def get_hard_output_scale(soft_a_s, soft_w_s, soft_a_out_s, hard_scale_method, input_scale_hard, weight_scale_hard, output_scale_hard, shift_num, out_scale_test):
    input_scale = soft_a_s
    weight_scale = soft_w_s
    output_scale = soft_a_out_s
    scale_temp = input_scale_hard * weight_scale_hard * output_scale_hard
    if hard_scale_method == 0:
