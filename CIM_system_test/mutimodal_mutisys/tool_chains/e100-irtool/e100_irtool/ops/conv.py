...
class GroupDotOp(AbsDotOp):
    ...
    def validate(self):
        ...
        if self.in_channel is not None:
            assert self.in_channel % self.group == 0, f'invalid group={self.group} for in_channel={self.in_channel}'
        if self.out_channel is not None:
            assert self.out_channel % self.group == 0, f'invalid group={self.group} for out_channel={self.out_channel}'
...
class ConvOp(AbsKernelOp, GroupDotOp):
    ...
    def weight_shapes(self, channel_last=False, **kwargs):
        ...
        return dict(weight=(*k, ci, co) if channel_last else (co, ci, *k), bias=None if not self.bias else (co,))
...
class ConvTransposeOp(AbsKernelOp, GroupDotOp):
    ...
    def __init__(self, *, output_padding=None, **kwargs):
        super().__init__(**kwargs)
        self.set_attr('output_padding', to_int_tuple(output_padding, keep_scalar=True), is_integers, min_val=0, ndims=(0, 1, self.ndim))

    def formalized_attrs(self):
        d = super().formalized_attrs()
        d['output_padding'] = to_int_tuple(self.output_padding, ndim=self.ndim)
        return d
