...
class DumpDataCmd(BaseCmd):
    ...
    def add_args(cls, argp):
        argp.add_argument('-p', '--print-data', action='store_true', help='print data to stdout')
    ...
    def show_item(self, name, value):
        self.info(f'  {name}:' if name else ' ', type(value).__qualname__, *self.show_array(value))
    ...
    def set_printoptions(self):
        for mod in ('numpy', 'torch'):
            if mod not in _print_optioned:
                ...
                if mod is not None:
                    try:
                        mod.set_printoptions(threshold=1000000)
