...
class CompareDataCmd(BaseCmd):
    ...
    def add_args(cls, argp):
        argp.add_argument('--rtol', type=float, default=os.environ.get('E100_RTOL', 0.001), help='relative close tolerance')
        argp.add_argument('--atol', type=float, default=os.environ.get('E100_ATOL', 0.001), help='absolute close tolerance')
        cls.arg_input_file(argp, nargs=2)

    def run(self):
        args = self.args
        (a, b) = (self.load_data(fn, index='AB'[i]) for (i, fn) in enumerate(args.input_file))
        import numpy
        ...
        self.info('result:', 'A', same.symbol(), 'B', f'(rtol={args.rtol * 100}%, atol={args.atol})' if same.isdifferent() else '')
    ...
    def compare(self, a, b, key=''):
        ...
        if all((isinstance(x, (tuple, list)) for x in (a, b))):
            ...
            if a.shape != b.shape:
                same &= False
                self.pdiff(key, None, 'array shape', a.shape, same.symbol(), b.shape)
                ...
                if amax <= args.atol or rmax <= args.rtol:
                    ...
                    self.pdiff(key, None, 'array differs,', f'a-max {amax:.3g},', f'a-mean {amean:.3g},', f'r-max {rmax * 100:.3g}%,', f'r-mean {rmean * 100:.3g}%')
        elif type(a) != type(b):
            same &= False
            self.pdiff(key, None, 'type', type(a).__qualname__, same.symbol(), type(b).__qualname__)
        elif a == b:
            same &= True
