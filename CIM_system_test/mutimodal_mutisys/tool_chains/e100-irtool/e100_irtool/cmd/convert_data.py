...
class BaseConvertCmd(BaseCmd):
    ...
    def add_args(cls, argp):
        argp.add_argument('-t', '--transpose-to', choices=cls.TRANSES, help='transpose tensors to channel last/first: NXC/NCX for inputs, XCC/CCX for weights')
    ...
    def trans(self, x, to=None):
        ...
        if to == 'NXC':
            ...
            if rank > 1:
                return self._be.transpose(x, (rank - 1, rank - 2, *range(0, rank - 2)))
        else:
            assert False
        return x
