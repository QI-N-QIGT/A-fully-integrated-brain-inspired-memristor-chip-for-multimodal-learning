...
class BaseGenDataCmd(BaseCmd):
    ...
    def add_args(cls, argp):
        ...
        argp.add_argument('-x', '--scale', type=float, default=0, help='value scale value, negative for signed')
        cls.arg_dims(argp)
    ...
    def prepare(self):
        from ..tools import infer_shapes
        args = self.args
    ...
    def gen_data(self, shape, dtype=None, batch=None, channel_last=None, signed=True):
        ...
        self.debug(shape, dtype, f'scale={scale}' if scale else '', 'channel_last' if channel_last else 'channel_first')
...
class GenInputCmd(BaseGenDataCmd):
    ...
    def add_args(cls, argp):
        super().add_args(argp)
        argp.add_argument('-n', '--batch', type=int, default=1, help='batch size')
    ...
    def run(self):
        ...
        with ir.with_layer(inp) as layer:
            for (i, dd) in layer.iter_inputs():
                s = dd.make_shape(dims=dims[i] if i < len(dims) else None, channel_last=channel_last)
