...
class RunIRCmd(BaseCmd):
    ...
    def add_args(cls, argp):
        ...
        argp.add_argument('-oa', '--output-all', action='store_true', help='save all data to output')
    ...
    def run(self):
        ...
        output = rt.run_ir(ir, inputs, weights, outputs=args.output_all or None, callback=self.callback)
        self.save_data(output)

    def callback(self, name, *, inputs, outputs, weights, **kwargs):
        if outputs is None:
            self.debug(f'inputs({len(inputs)}),', f'weights({len(weights)})')