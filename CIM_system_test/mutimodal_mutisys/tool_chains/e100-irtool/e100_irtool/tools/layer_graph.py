from e100_irtool.core import LayerTree, mixin
from e100_irtool.core.ref import parse_ref, make_ref
    ...
    def build_flat_graph(self):
        ...
        for (name, layer) in layers.items():
            ...
            if layer.is_layer_tree():
                grf.add_group(name, group=make_ref(*parts[:-1]), tag=layer.type, number=layer.number)
                continue
            if layer.type == 'input':
                ...
                pass
            grf.add_node(name, group=make_ref(*parts[:-1]), tag=layer.type, number=None)
        for (name, layer) in layers.items():
            parts = parse_ref(name)
            if layer.is_layer_tree():
                name = ins[name]
