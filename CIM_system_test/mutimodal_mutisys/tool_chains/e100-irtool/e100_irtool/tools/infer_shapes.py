...
from ..runtime.shape_inferer import ShapeInferer

@mixin(LayerTree)
class InferShapes:

    def infer_shapes(self, *shapes, dims_only=False, channel_last=None, inferer=None):
        if inferer is None:
            inferer = ShapeInferer()
        (inp, oup) = self.get_io_layers()
        with self.with_layer(inp) as layer:
            assert len(shapes) in (0, len(layer.inputs or ())), f'need {len(layer.inputs)} input shapes'
        ...
        for (name, layer) in self.iter_layers(deep=False, sorted=True):
            ...
            if layer.outputs is None:
                ...
                assert len(layer.outputs) == len(y), f'outputs length {len(layer.outputs)} != {len(y)}'
    ...
    def _infer_cl(self, layer):
        ...
        for (i, dd) in layer.iter_inputs():
            if dd.channel_last is None:
                ...
                assert dd.channel_last == channel_last, f'channel_last {dd.channel_last} != {channel_last}'
        return channel_last
    ...
    def _infer_wts(self, layer, *shapes, channel_last):
        ...
        if not channel:
            ...
            channel = None
        wts = layer.op.weight_shapes(channel_last=channel_last, channel=channel)
        wts = {k: v for (k, v) in wts.items() if v is not None}
        if not wts:
            assert not layer.weights, f'weights {set(layer.weights)} are invalid'
        elif layer.weights is None:
            layer.weights = {k: DataDef(shape=v) for (k, v) in wts.items()}
        else:
            assert set(wts) == set(layer.weights), f'weights {set(layer.weights)} != {set(wts)}'
            for (k, dd) in layer.iter_weights():
                if dd.shape is None:
                    dd.shape = wts[k]
                else:
                    assert dd.shape == wts[k], f'shape {dd.shape} != {wts[k]}'
                if dd.channel_last is not None:
