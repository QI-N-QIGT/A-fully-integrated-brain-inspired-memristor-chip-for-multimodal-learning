...
def is_integer(val, *, min_val=None, max_val=None):
    return type(val) is int and (min_val is None or val >= min_val) and (max_val is None or val <= max_val)

def is_number(val, *, min_val=None, max_val=None, lower_limit=None, upper_limit=None):
    return isinstance(val, (int, float)) and (min_val is None or val >= min_val) and (max_val is None or val <= max_val) and (lower_limit is None or val > lower_limit) and (upper_limit is None or val < upper_limit)

def is_one_or_more(obj, validator, *, min_dim=0, max_dim=None, ndims=None, none_ok=False, **kwargs):
    if obj is None:
        return none_ok
...
def mixin(base_cls):
    ...
    def wrapper(cls):
        ...
        return cls
    return wrapper
RE_VAR_TOKEN = re.compile('[^a-zA-Z0-9_]')
