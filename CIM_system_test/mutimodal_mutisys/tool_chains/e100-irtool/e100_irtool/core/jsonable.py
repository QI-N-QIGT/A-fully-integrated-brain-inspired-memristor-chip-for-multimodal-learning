    ...
    with circular_check(_ids, obj) as _ids:
        if isinstance(obj, Mapping):
            if filter is None or filter is False:
                ...
                pass
            res = {k: to_json_obj(v, filter=filter, _ids=_ids) for (k, v) in obj.items() if filter(k, v) and (not k.endswith('_'))}
            ...
            raise TypeError(f'can not convert {type(obj).__qualname__} to json type')
    return res
...
class Jsonable:
    ...
    def set_attr(self, name, value, valid=True, *, not_none=False, **kwargs):
        ...
        if not valid:
            raise ValueError(f'invalid attr {name}={value!r} for {self.__class__.__qualname__}')
