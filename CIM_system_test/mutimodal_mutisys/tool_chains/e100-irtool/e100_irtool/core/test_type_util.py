...
from .type_util import is_scalar, to_tokens, to_cls_obj, to_obj_dict, to_int_tuple, to_obj_list, is_integers, is_integer, is_boolean, to_boolean
...
class TestTypeUtil(TestCase):
    ...
    def test_to_boolean(self):
        for x in (None, False, 0, 0.0, '0', '', 'false', 'False', 'FALSE', 'no', 'N', [], {}):
    ...
    def test_to_cls_obj(self):
        ...
        self.assertEqual(to_cls_obj('1 2 3'.split(), str, func=lambda *x: ','.join(map(str, x))), '1,2,3')
        a = (False, 1, {'x': None})
    ...
    def test_to_obj_dict(self):
        ...
        self.assertEqual(to_obj_dict({'a': {'b': 'xx'}, 'x': None}, Foo), {'a': Foo(b='xx'), 'x': None})
        self.assertEqual(to_obj_dict([{'b': 3.14}, {'a': [2.18]}], Foo), [Foo(b=3.14), Foo(a=[2.18])])
    ...
    def test_obj_to_list(self):
        ...
        self.assertEqual(to_obj_list(({'b': 3}, {'a': [False, 0.4], 'b': {}}), Foo), [Foo(9, 3), Foo([False, 0.4], {})])
