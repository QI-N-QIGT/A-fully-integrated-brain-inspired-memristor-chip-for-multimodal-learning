...
class RTTests:
    ...
    def t(self, op_id, *in_shapes, batch=9, channel=None, **kwargs):
        x1 = [self._rand((batch, *s) if channel is None else self.numpy.to_axes(batch, channel, s)) for s in in_shapes]
        ...
        if op_id == 'split':
            ...
            self.assertTrue(numpy.allclose(y1, y2, atol=self.atol, rtol=self.rtol), msg=f'\nx\n:{x1}\nnumpy:\n{y1}\n{self.other.name}\n:{y2}\n')
    ...
    def test_op_coverage(self):
        ...
        for op_id in enum_op_ids():
            ...
            for rt in rts:
                ...
                self.assertIsNotNone(f, msg=f'{type(rt).__qualname__}.{fn} not exist')
    ...
    def test_norms(self):
        ...
        run('instance')
        kwargs.update(input_mean=self._rand([c]), input_var=self._rand([c], signed=False))
        run('batch')
    ...
    def test_matmul(self):
        (ci, co) = (11, 7)
        kwargs = dict(weight=self._rand([ci, co] if self.numpy.channel_last else [co, ci]), bias=self._rand([co]))
