...
from .utils import to_auto_pad, conv_t_shapes, resize_size_scale, concat_axis, split_to_secs

class TensorflowRuntime(MathRuntime):
    name = 'tensorflow'
    channel_last = True
    tensor_is_readonly = True
    DATA_FORMATS = (('NCW', 'NWC'), ('NCHW', 'NHWC'), ('NCDHW', 'NDHWC'))
    ...
    def fn_batch_norm(self, x, *, scale, bias, input_mean, input_var, epsilon):
        ...
        return self._be.nn.batch_normalization(x, input_mean, input_var, bias, scale, epsilon)
    ...
    def _pool(self, func, ndim, x, *, kernel, stride, padding, dilation, ceil_mode, auto_pad):
        ...
        assert len(x.shape) == ndim + 2, f'invalid input rank {len(x.shape)} for {func}_pool{ndim}d'
        if auto_pad is None:
        ...
        return fn(x, kernel, strides=stride, padding=auto_pad, data_format=self.data_format(ndim))
    ...
    def fn_matmul(self, x, *, weight, bias):
        y = self._be.matmul(x, weight if self.channel_last else self._be.transpose(weight))
        if bias is not None:
            y += bias
        return y
    fn_linear = fn_matmul
    ...
    def _conv(self, ndim, x, *, weight, bias, stride, padding, dilation, group, auto_pad):
        ...
        assert len(x.shape) == ndim + 2, f'invalid input rank {len(x.shape)} for conv{ndim}d'
        ...
        y = fn(x, weight, stride, padding, dilations=dilation, data_format=self.data_format(ndim))
        if bias is not None:
    ...
    def _conv_t(self, ndim, x, *, weight, bias, stride, padding, dilation, output_padding, group, auto_pad):
        ...
        (os, (_, _, p, *_)) = conv_t_shapes(xd, kernel, stride, padding, dilation, output_padding, auto_pad)
        if auto_pad is not None:
            assert padding is None
            padding = auto_pad
        elif ndim != 2:
        ...
        y = fn(x, weight, self.to_axes(ba, co, os), strides=stride, padding=padding, dilations=dilation, data_format=self.data_format(ndim))
        if bias is not None:
            y += self.broadcast(bias, len(y.shape), ndim)
        return y
    ...
    def fn_resize(self, x, *, size, scale, mode):
        ...
        assert all((a == b and a >= 1 for (a, b) in zip(sc, scale))), f'resize scale {sc} is not supported'
        if ndim == 1:
            scale = scale[0]
