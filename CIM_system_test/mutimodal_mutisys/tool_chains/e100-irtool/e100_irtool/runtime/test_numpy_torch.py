...
class TestNumpyTorchRuntime(RTTests, TestCase):
    ...
    def test_concat(self):
        ...
        self.t('concat', [3, 2], [3, 3], [3, 1], axis=1, with_batch=False, channel_pos='first')
        self.t('concat', [3, 2], [3, 3], [3, 1], axis=2, with_batch=True, channel_pos='first')
        self.t('concat', [3, 2], [3, 3], [3, 1], axis=0, with_batch=False, channel_pos='last')
        self.t('concat', [3, 2], [3, 3], [3, 1], axis=1, with_batch=True, channel_pos='last')
    ...
    def test_conv(self):
        ...
        def run():
            self.t('conv1d', [w], weight=self._rand([co, ci, k1]), bias=self._rand([co]), **kwargs)
            self.t('conv2d', [h, w], weight=self._rand([co, ci, k1, k2]), bias=self._rand([co]), **kwargs)
            self.t('conv3d', [d, h, w], weight=self._rand([co, ci, k1, k2, k3]), bias=self._rand([co]), **kwargs)
        kwargs = dict(stride=1, padding=1, dilation=1, group=1, channel=ci, auto_pad=None)
        run()
    ...
    def test_conv_transpose(self):
        ...
        def run():
            self.t('conv_transpose1d', [w], weight=self._rand([ci, co, k1]), bias=self._rand([co]), **kwargs)
            self.t('conv_transpose2d', [h, w], weight=self._rand([ci, co, k1, k2]), bias=self._rand([co]), **kwargs)
            self.t('conv_transpose3d', [d, h, w], weight=self._rand([ci, co, k1, k2, k3]), bias=self._rand([co]), **kwargs)
        kwargs = dict(stride=1, padding=1, dilation=1, group=1, output_padding=0, channel=ci, auto_pad=None)
        run()
        kwargs.update(padding=0)
        run()
