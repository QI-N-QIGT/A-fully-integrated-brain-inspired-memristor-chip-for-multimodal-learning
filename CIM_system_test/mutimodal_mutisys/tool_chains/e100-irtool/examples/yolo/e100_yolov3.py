...
ANCHORS = {'yolov3': ((10, 13, 16, 30, 33, 23), (30, 61, 62, 45, 59, 119), (116, 90, 156, 198, 373, 326)), 'yolov3-tiny': ((10, 14, 23, 27, 37, 58), (81, 82, 135, 169, 344, 319))}
STRIDES = {'yolov3': (8, 16, 32), 'yolov3-tiny': (16, 32)}
...
class Yolov3XywhOp(UnaryOp):
    ...
    def infer_shape(self, x, *, channel_last):
        (no, na, ny, nx, ci, co) = self._shapes(x, self.nclass, self.anchors, channel_last)
        return (co, no)
    ...
    def run_op(self, x, *, nclass, anchors, stride, runtime):
        (no, na, ny, nx, ci, co) = self._shapes(x.shape[1:], nclass, anchors, runtime.channel_last)
        if self._grid is None or self._grid.shape != (co,):
            self.make_grid((ny, nx), anchors, stride, runtime=runtime, dtype=x.dtype)
        ba = x.shape[0]
        if runtime.channel_last:
            x = runtime.fn_reshape(x, shape=(ba, ny, nx, na, no), with_batch=True)
            x = runtime.fn_transpose(x, perm=(0, 3, 1, 2, 4), with_batch=True)
            y = runtime.fn_sigmoid(x)
        else:
            x = runtime.fn_reshape(x, shape=(ba, na, no, ny, nx), with_batch=True)
        ...
        if runtime.tensor_is_readonly:
            y = runtime.fn_concat(xy, wh, y[..., 4:], axis=-1, with_batch=True, channel_pos='ignore')
        else:
            y[..., 0:2] = xy
            y[..., 2:4] = wh
    ...
    def make_grid(self, dims, anchors, stride, runtime, dtype):
        import numpy
        (ny, nx) = dims
        (yv, xv) = numpy.meshgrid(numpy.arange(ny), numpy.arange(nx), indexing='ij')
...
class Yolov3IR(BaseIR):
    ...
    def infer_strides(self):
        from e100_irtool.tools import infer_shapes
        s = 256
        ir = self.clone()
        ...
        for (name, layer) in d1.layers.items():
            if name.startswith('xywh-'):
                ...
                if op.stride is None:
                    ...
                    assert op.stride == stride, f'layer detect/{name}.stride {op.stride} != {stride}'
        return strides
    ...
    def add_tiny_layers(self):
        ...
        self.add_layer('b11', op=make_op('pad', pads=[0, 0, 1, 1]), inputs=['b10'])
        ...
        self.add_layer('detect', layer=self._detect('yolov3-tiny', 256, 512), inputs=['b19', 'b15'])
    ...
    def add_full_layers(self):
        ...
        self.add_layer('b17', op=self._usmp(2), inputs=['b16'])
        self.add_layer('b18', op=self._concat(1), inputs=['b17', 'b8'])
        self.add_layer('b19', layer=self._bnck(512, 1, False, c1=768), inputs=['b18'])
        ...
        self.add_layer('b26', layer=self._bnck(256, 1, False, c1=384), inputs=['b25'])
        self.add_layer('b27', layer=self._bnck(256, 2, False), inputs=['b26'])
        self.add_layer('detect', layer=self._detect('yolov3', 256, 512, 1024), inputs=['b27', 'b22', 'b15'])

    def _conv(self, ci, co, k, s=None, fused=True):
        b = make_layer(type='block')
        b.add_layer('inp', type='input', inputs=[dict(channel=ci)])
        b.add_layer('conv', op=make_op('conv2d', in_channel=ci, out_channel=co, kernel=k, stride=s, padding=k // 2), inputs=['inp'])
        if not fused:
            b.add_layer('bn', op=make_op('batch_norm2d', channel=co), inputs=['conv'])
    ...
    def _detect(self, name, *ci, nc=80):
        ...
        for (i, c) in enumerate(ci):
            b.add_layer(f'conv-{i}', op=make_op('conv2d', in_channel=c, out_channel=no * na, kernel=1), inputs=[f'inp:{i}'])
            b.add_layer(f'xywh-{i}', op=make_op('yolov3-xywh', nclass=nc, anchors=anchors[i], stride=strides[i]), inputs=[f'conv-{i}'])
        b.add_layer('concat', op=self._concat(1, channel_pos='ignore'), inputs=[f'xywh-{i}' for i in range(len(ci))])
