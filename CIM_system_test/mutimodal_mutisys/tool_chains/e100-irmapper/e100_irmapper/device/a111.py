...
class A111MappingInfo(Jsonable):
    ...
    def __init__(self, *, col_split_num=1, row_split_num=1, col_repeat_num=1, row_repeat_num=1, para_diff_array=1, input_buffer_addr=None, output_buffer_addr=None, in_buf_type=0, out_buf_type=0, mappings=None, **kwargs):
...
class A111CalcInfo(Jsonable):
    ...
    def __init__(self, *, weight_scale=1, adc_range=1, relu_threshold=0, shift_num=0, bias_input=None, buffer_wrap_en=0, output_padding=None, **kwargs):
...
class A111TileInfo(Jsonable):
    ...
    def __init__(self, *, tile_mode=3, pool0_en=0, pool1_en=0, pool2_en=0, pool3_en=0, op_list=None, in_esram_addr=None, out_esram_addr=None, **kwargs):
...
class A111TileDevice(BaseDevice):
    kind = 'a111-tile'
    devices = {'xb': {'kind': 'a111-xb', 'number': 4}}
    info = A111TileInfo(tile_mode=3, pool0_en=0, pool1_en=0, pool2_en=0, pool3_en=0, op_list=None)
...
class A111XbDevice(RramDevice):
    kind = 'a111-xb'
    profile = dict(in_channel=320, out_channel=128, in_bits=8, out_bits=8, weight_bits=4, signed=True)
...
class A111NpuDevice(BaseDevice):
    kind = 'a111-npu'
    devices = {'a111-tile:0': {'kind': 'a111-tile'}, 'a111-tile:1': {'kind': 'a111-tile'}, 'a111-tile:2': {'kind': 'a111-tile'}, 'a111-tile:3': {'kind': 'a111-tile'}, 'a111-tile:4': {'kind': 'a111-tile'}, 'a111-tile:5': {'kind': 'a111-tile'}, 'a111-tile:6': {'kind': 'a111-tile'}, 'a111-tile:7': {'kind': 'a111-tile'}, 'a111-tile:8': {'kind': 'a111-tile'}, 'a111-tile:9': {'kind': 'a111-tile'}, 'a111-tile:10': {'kind': 'a111-tile'}, 'a111-tile:11': {'kind': 'a111-tile'}, 'mcu': {'kind': 'a111-mcu'}}
    ip = None
