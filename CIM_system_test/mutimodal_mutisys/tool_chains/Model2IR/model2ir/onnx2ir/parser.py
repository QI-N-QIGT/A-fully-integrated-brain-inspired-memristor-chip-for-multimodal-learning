...
class OnnxParser(object):
    ...
    def __init__(self, onnx_model, weight_half_level=None, weight_scale=None, data_clamp_std=0, data_range_specify=None):
                        ...
                        if self.weight_half_level != None:
                            if 'bias' in node_name:
                                if '_lstm_bias' in node_name:
                                    for n in ['-input_gate_x-0.bias', '-output_gate_x-0.bias', '-forget_gate_x-0.bias', '-cell_state_x-0.bias', '-input_gate_h-0.bias', '-output_gate_h-0.bias', '-forget_gate_h-0.bias', '-cell_state_h-0.bias']:
                                ...
                                if '_lstm_x' in node_name:
                                    for n in ['-input_gate_x-0.weight', '-output_gate_x-0.weight', '-forget_gate_x-0.weight', '-cell_state_x-0.weight']:
                                        (tensor, scale) = data_quantization_sym(self.weight_numpy[f'{layer_name}{n}'], half_level=self.weight_half_level, data_range=self.data_range_specify, clamp_std=self.data_clamp_std, isint=1)
                                    ...
                                    (tensor, scale) = data_quantization_sym(self.weight_numpy[node_name], half_level=self.weight_half_level, data_range=self.data_range_specify, clamp_std=self.data_clamp_std, isint=1)
