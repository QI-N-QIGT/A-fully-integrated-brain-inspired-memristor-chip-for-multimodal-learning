...
class ResizeOp(UnaryOp):
    ...
    def __init__(self, *, size=None, scale=None, mode=None, **kwargs):
        ...
        assert (size is None) ^ (scale is None), f'invalid size {size!r} and scale {scale!r}'
        self.set_attr('size', to_int_tuple(size, keep_scalar=True), is_integers, min_val=1, min_dim=0, max_dim=4)
        self.set_attr('scale', scale, is_numbers, lower_limit=0, min_dim=0, max_dim=4)
