    ...
    def validate_graph(self):
        ...
        with self.with_layer(inp) as layer:
            ...
            for (i, dd) in enumerate(layer.inputs):
                if dd.loop == 'output':
                    assert dd.index >= 0 and dd.index < noups, f'invalid input[{i}].index not in [0, {noups})'
        ...
        with self.with_layer(inp) as layer:
            ...
            for (i, dd) in enumerate(layer.inputs):
                ...
                if dd.loop is None:
                    ...
                    ol = self.layers[oup]
                    oi = dd.loop.index
                    inputs.append(('O', di.ref, *ol.inputs[oi].parse_ref()))
                else:
    ...
    def infer_shapes(self, *shapes, **kwargs):
        ...
        for (i, dd) in enumerate(layer.inputs):
            ...
            if lo.source == 'split':
                ...
                n = self.repeat
                sa = s[a] // n
                assert a >= 0 and a < len(s) and (sa * n == s[a]), f'invalid split {s}[{a}] to {n} secs'
                ins[i] = (*s[:a], sa, *s[a + 1:])
