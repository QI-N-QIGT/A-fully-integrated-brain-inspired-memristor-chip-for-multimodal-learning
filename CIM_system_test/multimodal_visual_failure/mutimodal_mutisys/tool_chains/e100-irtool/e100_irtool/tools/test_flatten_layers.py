...
class TestFlattenLayers(TestCase):
    ...
    def test_flatten_tree_1(self):
        ...
        self.assertEqual(to_json_obj(layers), {'b-x': {'op': {'op_id': 'add'}, 'inputs': ['inp:0', 'inp:1']}})
        b.number = 2
        (oups, layers) = b.flatten_tree('b')
        self.assertEqual(to_json_obj(oups), ['b-1-x', 'inp:1'])
        self.assertEqual(to_json_obj(layers), {'b-0-x': {'op': {'op_id': 'add'}, 'inputs': ['inp:0', 'inp:1']}, 'b-1-x': {'op': {'op_id': 'add'}, 'inputs': ['b-0-x', 'inp:1']}})
    ...
    def test_flatten_tree_2(self):
        ...
        self.assertEqual(to_json_obj(layers), {'b-b1-x': {'op': {'op_id': 'add'}, 'inputs': ['inp2:1', 'inp2:0']}})
        b.get_layer('b1').number = 2
        b.number = 2
        (oups, layers) = b.flatten_tree('b')
        self.assertEqual(to_json_obj(oups), ['b-1-b1-1-x', 'b-0-b1-1-x'])
        self.assertEqual(to_json_obj(layers), {'b-0-b1-0-x': {'op': {'op_id': 'add'}, 'inputs': ['inp2:1', 'inp2:0']}, 'b-0-b1-1-x': {'op': {'op_id': 'add'}, 'inputs': ['inp2:1', 'b-0-b1-0-x']}, 'b-1-b1-0-x': {'op': {'op_id': 'add'}, 'inputs': ['inp2:0', 'b-0-b1-1-x']}, 'b-1-b1-1-x': {'op': {'op_id': 'add'}, 'inputs': ['inp2:0', 'b-1-b1-0-x']}})
