...
class NumpyRuntime(MathRuntime):
    ...
    def fn_selu(self, x, *, alpha, gamma):
        return self._be.where(x < 0, (self._be.exp(x) - 1) * alpha * gamma, x * gamma)
    ...
    def _kerneled(self, x, *, ndim, func, kernel, stride, padding, dilation, ceil_mode=False, dont_pad=False, o_channel=None, **kwargs):
        bc = len(x.shape) - ndim
        ...
        os = [(xs[bc + i] + p[i] + p[ndim + i] - dk[i] + c[i]) // s[i] + 1 for i in range(ndim)]
        if ceil_mode:
            for i in range(ndim):
        ...
        for oi in product(*map(range, os)):
            ii = tuple((slice(max(0, oi[i] * s[i] - pl[i]), oi[i] * s[i] - pl[i] + dk[i], d[i]) for i in range(ndim)))
    ...
    def _conv(self, x, ndim, *, weight, bias, stride, padding, dilation, group, auto_pad):
        numpy = self._be
