...
class CIMAConvLayer(CIMAComputeBaseLayer):
    ...
    def __init__(self, layer_name, *, core_id=[0, 0], pe_cluster_id=0, pe_xb_id=0, in_channel=0, out_channel=0, relu=False, kernel_size=1, stride=1, padding=0, silu=False, credit_len=0):
...
class CIMAFCLayer(CIMAComputeBaseLayer):
    ...
    def __init__(self, layer_name, *, core_id=[0, 0], pe_cluster_id=0, pe_xb_id=0, in_channel=0, out_channel=0, relu=False, silu=False, credit_len=0):
...
class CIMAPoolingLayer(CIMAComputeBaseLayer):
    ...
    def __init__(self, layer_name, *, core_id=[0, 0], in_channel=0, out_channel=0, pool_kernel=2, pool_mode='max', pool_stride=2, credit_len=0):
...
class CIMAPipeGraph:
            ...
            for nl in current_layer_name:
                ...
                if isinstance(current_layer_info, CIMAConvLayer):
                    ...
                    if current_layer_info.credit_len == 0:
                        ...
                        current_layer_info.set_credit_len(credit_len)
                elif isinstance(current_layer_info, CIMAConcatLayer) or isinstance(current_layer_info, CIMAAddLayer) or isinstance(current_layer_info, CIMASplitLayer) or isinstance(current_layer_info, CIMAIdentityLayer):
                    current_layer_info.set_input_image_size(current_image_size)
                    current_layer_info.set_output_image_size(current_image_size)
    ...
    def to_ir(self):
        ...
        for l in self.layer_info_list:
            if isinstance(l, CIMAInputLayer) or isinstance(l, CIMAOutputLayer):
                ...
                if op_layer.relu or op_layer.avgpool:
                    ...
                    op_ = make_op(op_id, in_channel=op_layer.in_channel, out_channel=op_layer.out_channel, kernel=op_layer.kernel_size, stride=op_layer.stride, padding=op_layer.padding, relu=relu, silu=silu)
                else:
                    op_ = make_op(op_id, in_channel=op_layer.in_channel, out_channel=op_layer.out_channel, kernel=op_layer.kernel_size, stride=op_layer.stride, padding=op_layer.padding)
                in_info = []
                index = 0
                for nl in op_layer.ref_name:
                    if 'graph_input' in nl:
                        ...
                        graph_input_count += 1
                    in_info.append(dict(ref=nl, channel=op_layer.in_channel, height=op_layer.input_image_size[index][0], width=op_layer.input_image_size[index][1]))
                    index += 1
                weight_info = dict(weight=dict(shape=op_layer.weight_shape))
                out_info = [dict(channel=op_layer.out_channel, height=op_layer.output_image_size[0][0], width=op_layer.output_image_size[0][1])]
                ...
                CIMA_mapping_info = CIMAMappingInfo(in_line_buffer_addr=[[hex(op_layer.in_buffer_addr[0]), hex(op_layer.in_buffer_addr[1])]], credit_len=[op_layer.credit_len], mappings=mapping_info)
                ...
                for nl in op_layer.ref_name:
                    ...
                    in_info.append(dict(ref=nl, channel=ic, height=op_layer.input_image_size[index][0], width=op_layer.input_image_size[index][1]))
                    index += 1
                if op_id == 'fused_concat':
                    out_info = []
                    for oc in op_layer.out_channel:
                        out_info.append(dict(channel=oc, height=op_layer.input_image_size[index][0], width=op_layer.input_image_size[index][1]))
                        index += 1
                else:
                    out_info = [dict(channel=op_layer.out_channel, height=op_layer.output_image_size[0][0], width=op_layer.output_image_size[0][1])]
                ...
                in_info.append(dict(ref=ref_name, channel=op_layer.in_channel, height=op_layer.input_image_size[0][0], width=op_layer.input_image_size[0][1]))
                out_info = [dict(channel=op_layer.out_channel, height=op_layer.output_image_size[0][0], width=op_layer.output_image_size[0][1])]
                current_layer = make_layer(op=op_, inputs=in_info, outputs=out_info)
                mapping_info = []
                core_index = op_layer.core_id[0] * 6 + op_layer.core_id[1]
                ...
                in_info = []
                assert len(op_layer.ref_name) == 1, f'identity op 的ref name应该只有一个!!! {op_layer.ref_name}'
                in_info.append(dict(ref=nl, channel=op_layer.in_channel, height=op_layer.input_image_size[0][0], width=op_layer.input_image_size[0][1]))
                for oc in op_layer.out_channel:
                    out_info = [dict(channel=oc, height=op_layer.output_image_size[0][0], width=op_layer.output_image_size[0][1])]
                current_layer = make_layer(op=op_, inputs=in_info, outputs=out_info)
                mapping_info = []
                core_index = op_layer.core_id[0] * 6 + op_layer.core_id[1]
                device_ref = f'cima-0.cima-node:{core_index}'
                ...
                if op_id == 'fused_add':
                    out_info = []
                    for oc in op_layer.out_channel:
                        in_info.append(dict(channel=oc, height=op_layer.input_image_size[index][0], width=op_layer.input_image_size[index][1]))
                        index += 1
                else:
                    out_info = [dict(channel=op_layer.out_channel, height=op_layer.output_image_size[0][0], width=op_layer.output_image_size[0][1])]
                current_layer = make_layer(op=op_, inputs=in_info, outputs=out_info)
        ...
        for l in out_layer.ref_name:
            ...
            outputs_dict = dict(ref=ref_layer_info.layer_name, channel=ref_layer_info.out_channel, height=ref_layer_info.output_image_size[0][1], width=ref_layer_info.output_image_size[0][1])
            inputs.append(outputs_dict)
        self.ir.add_layer('graph_output', type='output', inputs=inputs)
        self.ir.add_device(name='cima-0', kind='cima-node', num=36, height=6, width=6, task_num=128)
        return self.ir

    def to_systemc_config(self, config_path, log_level=0, trace_enable=True, perf_single_trace=False, run_time=1000000, batch=1):
