...
def draw_rectangles(H, W, rectangles, save_fig=False):
    ...
    if n > 1:
        fig.set_size_inches(6* (n - 1), 5)  # 设置图像大小
    else:
    ...
    for key,value in rectangles.items():
        if n > 1:
            ...
            ax[c].set_xticks([0, W])
            ax[c].set_yticks([0, H])
            ax[c].set_aspect('auto')  # 设置纵横比
        else:
            ax.set_xlim(0, W)
            ax.set_ylim(H, 0)
            ax.set_xticks([0, W])
            ax.set_yticks([0, H])
            ax.set_aspect('auto')  # 设置纵横比
        colors = ['lightblue', 'lightgreen', 'lightyellow', 'lightcoral',
                'lightsalmon', 'lightpink', 'lightgray', 'lightcyan']
...
if __name__ == "__main__":
    ...
    # onnx转ir
    onnx_obj = ConvertONNX(model, weight_half_level = 7,
    ...
    map = mapper(ir=onnx_ir,device=device,
                cpu_layer = cpu_layer,
                calc_info=calc_info,
                place_strategy=LowestLevelAlgorithm,
                average_copy=copy_para,
                runtime='simulation',)
    map.run()
    mapped_ir = map.ir
    # mapped_ir.dump_json(file=f'resnet-56.yaml')
