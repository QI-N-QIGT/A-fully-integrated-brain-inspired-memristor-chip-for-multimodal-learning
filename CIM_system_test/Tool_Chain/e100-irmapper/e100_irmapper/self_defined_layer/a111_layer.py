...
class A111ComputeBaseLayer:
    ...
    def __init__(self, layer_name, tile_id=0, xb_id_list=[0], in_channel=0, 
                       out_channel=0, adc_range=1, relu=False, shift_num=1,
                       weight_scale=1, relu_threshold=0):
...
class A111ConvLayer(A111ComputeBaseLayer):
    ...
    def __init__(self, layer_name, *, tile_id=0, xb_id_list=[0], in_channel=0, 
                       out_channel=0, adc_range=1, relu=False, avgpool=False,
                       shift_num=1, weight_scale=1, kernel_size = 1, stride = 1,
                       output_pad = [0,0,0,0], relu_threshold=0):
...
class A111FCLayer(A111ComputeBaseLayer):
    ...
    def __init__(self, layer_name, *, tile_id=0, xb_id_list=[0], in_channel=0, 
        ...
        super().__init__(layer_name, tile_id=tile_id, xb_id_list=xb_id_list, 
                         in_channel=in_channel, out_channel=out_channel, 
                         adc_range=adc_range, relu=relu, shift_num=shift_num, 
                         weight_scale=weight_scale, relu_threshold=relu_threshold)
...
class A111PipeGraph:
    ...
    def add_layer(self, layer):
        ...
        if isinstance(layer, A111InputLayer):
            ...
            # 如果只有一个层，表示此时为第一个计算层，
            # 需要添加当前层的ref_name， input_image_size, output_image_size
            if len(self.layers) == 1:
                ...
                in_addr = self.set_layer_in_buffer_addr( layer.tile_id, layer.xb_id_list[0],
                                                         ISSameTileWithLastLayer=ISSameTileWithLastLayer)
                layer.set_in_buffer_addr(in_addr)
    ...
    def set_layer_in_buffer_addr(self, tile_id, first_xb, ISInputLayer=False, ISSameTileWithLastLayer=False):
        '''
        return (基地址, buffer起始的地址, buffer结束地址)
        '''
        if ISInputLayer:
            return 0x78000000, 0, 0x4000
        else:
            if ISSameTileWithLastLayer:
                base_addr = 0x78000000
            else:
                assert tile_id >=0 and tile_id <= 11
                base_addr = 0x70000000 + 0x1000000 * (tile_id % 6)
            assert first_xb in [0, 2]
            start_addr = 0x0 + 0x4000 * (first_xb // 2)
            end_addr = 0x4000 + start_addr
            ...
            if isinstance(l, A111ConvLayer):
                ...
                if op_layer.relu or op_layer.avgpool:
                    ...
                    op_ = make_op(op_id, in_channel=op_layer.in_channel,
                                ...
                                stride=op_layer.stride, 
                                padding=0)
                in_info = [dict(ref=op_layer.ref_name,channel=op_layer.in_channel,
                               height=op_layer.input_image_size[0],width=op_layer.input_image_size[1])]
                weight_info = dict(weight=dict(shape=op_layer.weight_shape))
                out_info = [dict(channel=op_layer.out_channel, height=op_layer.output_image_size[0], 
                                width=op_layer.output_image_size[1])]
                current_layer = make_layer(op=op_, inputs=in_info, outputs=out_info, weights=weight_info)
                
                # 设置当前的 mapping info
                in_buffer_addr = dict(base=hex(op_layer.in_buffer_addr[0]), start=hex(op_layer.in_buffer_addr[1]),
                                      end=hex(op_layer.in_buffer_addr[2]))
                out_buffer_addr = dict(base=hex(op_layer.out_buffer_addr[0]), start=hex(op_layer.out_buffer_addr[1]),
                                      end=hex(op_layer.out_buffer_addr[2]))
                mapping_info = []
                c = 0
                ...
                for xb in op_layer.xb_id_list:
                    ...
                    c += 1
                a111_mapping_info = A111MappingInfo(row_split_num=len(op_layer.xb_id_list), 
                                                        input_buffer_addr = in_buffer_addr, 
                                                        output_buffer_addr = out_buffer_addr,
                                                        in_buf_type = 6, out_buf_type= 6,
                                                        mappings = mapping_info)
                out_pad = {'top':op_layer.output_pad[0], 'bottom':op_layer.output_pad[1], 
                           'left':op_layer.output_pad[2], 'right':op_layer.output_pad[3]}
                buffer_wrap = 1
                if self.layer_graph[op_layer.layer_name] == 'graph_output':
                    buffer_wrap = 0
                a111_calc_info = A111CalcInfo(weight_scale=op_layer.weight_scale, adc_range=op_layer.adc_range,
                                                  relu_threshold=op_layer.relu_threshold, shift_num=op_layer.shift_num,
                                                  buffer_wrap_en=buffer_wrap, output_padding=out_pad)
                self.ir.add_layer(op_layer.layer_name, current_layer, a111_mapping_info=a111_mapping_info, 
                                  a111_calc_info=a111_calc_info,)
                ...
                current_layer = make_layer(op=op_, inputs=in_info, outputs=out_info, weights=weight_info)
                # 设置当前的 mapping info
                in_buffer_addr = dict(base=op_layer.in_buffer_addr[0], start=op_layer.in_buffer_addr[1],
                                      end=op_layer.in_buffer_addr[2])
                out_buffer_addr = dict(base=op_layer.out_buffer_addr[0], start=op_layer.out_buffer_addr[1],
                                      end=op_layer.out_buffer_addr[2])
                mapping_info = []
                c = 0
                for xb in op_layer.xb_id_list:
                    device_ref = f'a111-0.a111-npu:0.a111-tile:{op_layer.tile_id}.a111-xb:{xb}'
