...
if __name__ == "__main__":
    ...
    # copy_para = None
    # 声明仿真计算时各层的参数
    calc_info = C200CalcInfo(shift_expansion_mode = 'bit_pulse',
                             output_half_level=31, adc_clamp= False,
                             adc_quant = False, noise_scale=0.00)
    ...
    # test_input = np.load(model_path+'dataset\\test_input.npy')
    ...
    # 初始化runtime
    rt = C200NumpyRT(mapped_array_data=array_data,
                        device_adc_scale_data=device_adc_scale_data,
                        quant_all=True,
                        multi_batch=True,
                        half_level=31)
    ...
    if calc:
        for batch_size in [1, 10, 100, 1000, 10000]:
            ...
            time2 = time.time()
            print(f'batch size :{batch_size}; calculation time: {time2 - time1} s')
            data_output = np.concatenate(data_output,axis=0)
            # print(f'串行 layer calc time: {rt.layer_calc_time}')
