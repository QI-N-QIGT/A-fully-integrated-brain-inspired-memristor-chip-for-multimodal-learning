...
class LSQ_weight_quantizer(nn.Module):
    def __init__(self, *, bit, s, thd_neg, thd_pos, all_positive=False, symmetric=False, per_channel=False, noise_scale=0,
                    noise_method='add', noise_range='max', s_init=2,
                    init_mode='origin', init_percent=0.95, int_flag=False):
        # bit -> quant bits
...
class LSQ_act_quantizer(nn.Module):
    def __init__(self, *, bit, thd_neg, thd_pos, s, per_channel=False,
...
class Set_scale_quantizer(nn.Module):
    def __init__(self, bit=None, all_positive=False, symmetric=True, noise_scale=0, noise_method='add',
                 noise_range='max', int_flag=False):
...
def add_noise_channel_wise(weight, method = 'add', n_scale = 0.074, n_range = 'max'): # only for layer output with channel in index 1
    # weight -> input data, usually a weight
    # method ->
    #   'add' -> add a Gaussian noise to the weight, preferred method
    #   'mul' -> multiply a noise factor to the weight, rarely used
    ...
    for c in range(weight.shape[1]):
        ...
        if method == 'mul':
            weight_noise[:, c, :, :] = torch.randn_like(weight[:, c, :, :], device=weight.device).clamp_(-3.0, 3.0) * n_scale[c] + 1 ## whether clamp randn to (-3,3)
            weight_noise[:, c, :, :] = weight[:, c, :, :] * weight_noise[:, c, :, :]
    weight_noise = (weight_noise - weight).detach() + weight
    return weight_noise
