...
class conv2d_quant_noise(nn.Conv2d):
    def __init__(self, *, weight, bias, stride, padding, dilation,
                    ...
                    offset_vector, Gain_Error, hard_scale_method,
                    number_of_XB, output_clip, XB_lines,
                    # w_quantizer=None,
                    # a_quantizer=None,
        ...
        # self.number_of_XB = math.ceil(self.weight.reshape((out_channels, -1)).shape[1] / 320)
        # self.XB_lines = math.ceil(self.weight.reshape((out_channels, -1)).shape[1] / self.number_of_XB)
        self.number_of_XB = number_of_XB
        self.XB_lines = XB_lines
        # self.register_buffer('offset_vector', torch.zeros(out_channels))
        # self.register_buffer('Gain_Error', torch.ones(out_channels))
        self.offset_vector = offset_vector
        self.Gain_Error = Gain_Error
        # self.out_scale_test = out_scale_test
    ...
    def forward(self, input):
        ...
        if not self.A111_process:
            ...
            if self.A111_process == 2:
                input_int = torch.clamp(round_pass(input_int), 0, 255)  # 8bit全正输入
                input_int, input_scale = shift_clip_for_4bit_input(input_int, input_scale, self.input_method,
                                                                   self.input_shift,
                                                                   self.input_clip_bit)  # 4bit输入整型值
            ...
            if self.using_hard_scale == 1:
                ...
                # weight_scale_hard = self.G_max / weight_int.max()
                output_scale_hard = 2 ** self.ADC_bit / (self.ADC_scale * 2)
                output_scale = self.a_out_quantizer.get_scale()
                output_S = get_hard_output_scale(input_scale, weight_scale, output_scale,
                                                 self.hard_scale_method,
                                                 input_scale_hard, weight_scale_hard, output_scale_hard,
                                                 self.shift_num, True)
                input_int_t = F.unfold(input_int, self.weight.size()[-2:], dilation=self.dilation, padding=self.padding,
                                       stride=self.stride)
                weight_int_t = F.unfold(weight_int, self.weight.size()[-2:], dilation=self.dilation, padding=0,
                                        stride=self.stride)
                ...
                for i in range(0, self.number_of_XB):
                    ...
                    # 一块 xb上的运算 分为高低4bit运算和adc转化并合成
                    xbar_output = xbar_basic_block_hard_method(self.hard_scale_method, xbar_input,
                                                                                  input_scale, xbar_weight,
                                                                                  weight_scale, output_scale,
                                                                                  self.in_quant_low_bit, input_scale_hard,
                                                                                  weight_scale_hard, output_scale_hard)
                    output_tr += xbar_output
                input_size_stride = (int(math.sqrt(input_int_t.size()[1])), int(math.sqrt(input_int_t.size()[1])))
                # 输出变为 4-dim 数据 往后传播
                output = F.fold(output_tr.transpose(1, 2), input_size_stride, (1, 1), dilation=self.dilation, padding=0,
                                stride=(1, 1))
                ...
                for i in range(0, self.number_of_XB):
                    ...
                    xbar_weight = weight_int_t[:, i * self.XB_lines:(i + 1) * self.XB_lines, :]
                    # 一块 xb上的运算 分为高低4bit运算和adc转化并合成
                    xbar_output = xbar_basic_block_hard_method(1, xbar_input,
                                                              1.0, xbar_weight,
                                                              1.0, 1.0,
                                                              self.in_quant_low_bit, input_scale_hard,
                                                              weight_scale_hard, output_scale_hard)
                ...
                for i in range(0, self.number_of_XB):
                    ...
                    xbar_weight = weight_int_t[:, i * self.XB_lines:(i + 1) * self.XB_lines, :]
                    # 一块 xb上的运算 分为高低4bit运算和adc转化并合成
                    xbar_output, xbar_output_scale = xbar_basic_block_v2(xbar_input, input_scale, xbar_weight, weight_scale,
                                                                         self.in_quant_low_bit, self.a_out_quantizer,
                                                                         True)
                    output_tr += xbar_output
                input_size_stride = (int(math.sqrt(input_int_t.size()[1])), int(math.sqrt(input_int_t.size()[1])))
                # 输出变为 4-dim 数据 往后传播
        ...
        output = output * self.Gain_Error.unsqueeze(dim=1).unsqueeze(dim=1) + \
                 self.offset_vector.unsqueeze(dim=1).unsqueeze(dim=1)
        output = round_pass(output)
        # 以下，硬件推理不需要添加output noise
...
class linear_quant_noise(nn.Linear):
    def __init__(self,*, weight,bias,
                 ...
                 offset_vector, Gain_Error,
                 output_clip, number_of_XB, 
                 XB_lines,
                 hard_scale_method,
                 w_quantizer=None,
                 a_quantizer=None,
        ...
        self.number_of_XB = number_of_XB
        self.XB_lines = XB_lines
        # self.register_buffer('offset_vector', torch.zeros(out_features))
        # self.register_buffer('Gain_Error', torch.ones(out_features))
        self.offset_vector = offset_vector
        self.Gain_Error = Gain_Error
        # self.out_scale_test = out_scale_test
    ...
    def forward(self, input):
        ...
        if not self.A111_process:
            ...
            if self.using_hard_scale == 1:
                ...
                # lines = input_int.size()[1]
                weight_int_t = weight_int.t()
                output = 0
                # 将运算分到每一块 xb上进行运算  区别于conv
                for i in range(0, self.number_of_XB):
                    ...
                    # 一块 xb上的运算 分为高低4bit运算和adc转化并合成
                    xbar_output = xbar_basic_block_hard_method(self.hard_scale_method, xbar_input,
                                                                input_scale, xbar_weight, weight_scale,
                                                                output_scale,
                                                                self.in_quant_low_bit, input_scale_hard,
                                                                weight_scale_hard,
                                                                output_scale_hard)
...
class add_quant(nn.Module): # 输入输出都为[int, s]
    def __init__(self, * , thd_neg=0, thd_pos=1, bit=9, all_positive=False, symmetric=True, quant_method=0, shift=0, 
                         clip_before=False):
    ...
    def forward(self, input1, input2):
        if not (isinstance(input1, tuple) and isinstance(input2, tuple)):
            return input1 + input2  # float-32 add func
        # 浮点相加普通模式return input1 + input2 # float-32 add func
        if self.quant_method == -1:
            if isinstance(input1, tuple) and isinstance(input2, tuple):
                return (input1[0] * input1[1] + input2[0] * input2[1], 1)
            else:
                raise ValueError('add_quant module need tuple input. ')
...
class AdaptiveAvgPool2d_quant(nn.AdaptiveAvgPool2d):
    def __init__(self, *, thd_neg, thd_pos, quant_flag=False, bit=9, all_positive=False, symmetric=True,
                 set_shift_num=False, shift_num=0):
...
class AvgPool2d_quant(nn.AvgPool2d):
    def __init__(self, *, kernel, stride, padding, ceil_mode, quant_flag=False, bit=9, thd_neg=0, thd_pos=0,
                 set_shift_num=False, shift_num=0):
    ...
    def forward(self, input):
        if self.quant_flag:
            ...
            #          * self.size_2D
            output_int = F.avg_pool2d(input_int, self.kernel_size, self.stride,
                            self.padding, self.ceil_mode) * self.size_2D
