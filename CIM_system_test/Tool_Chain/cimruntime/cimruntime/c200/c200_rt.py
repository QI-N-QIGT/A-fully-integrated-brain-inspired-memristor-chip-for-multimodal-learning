...
from .utils import *
from e100_irmapper.device.c200 import MappedLayer # noqa
...
class C200NumpyRT(NumpyRuntime):
    ...
    def __init__(self,*,mapped_array_data=None,
        ...
        device_adc_scale_data : 
          设备的参数 存放{'array_id':[adc_scale],...}
        mapped_array_data:
          mapping好的数据，存放{'array_id':[tensor@576*128],...}
        '''
        super().__init__()
    ...
    def program(self, ir, array_data, layer_name = None, program_times = 3):
        ...
        for index in range(len(program_name)):
            print(f'当前写入权重层 ====> {program_name[index]}')
            ...
            for d in range(len(device[index])):
                ...
                for k, v in programming_array_data.items():
                    ...
                    if 'rram-144k' in k:
                        ...
                        # 转化成 rpc_api 的输入格式
                        # v_ = Uint8s(shape = v.shape, data = v)
    ...
    def read(self, ir, layer_name = None):
        ...
        for index in range(len(program_name)):
            ...
            print(f'当前读出权重层 ====> {program_name[index]}')
            assert len(device[index]) == len(addr[index])
            for d in range(len(device[index])):
                device_ = device[index][d]
                addr_ = addr[index][d]
    ...
    def fn_matmul(self, x, **kwargs):
        ...
        input:
          x 输入为 CIMTensor类型数据 
          输入 data 需要被量化为 half level范围内的定点数 --> quant_all = False
          输入 data 也有可能需要是浮点数 --> quant_all == True
        weight / bias:
          权重和bias数据也是 CIMTensor类型数据，同时数值大小是 half level 范围内的定点数，(外部量化完成)
        =================================================
        '''
        ...
        if 'device_info' not in kwargs.keys():
            if not self.quant_all:
                ...
                in_, in_scale = CNT.to_cimtensor_quant(data=in_d.data,half_level=self.half_level,
                                                       method=self.chip_quant_method,thr=self.thr,
                                                       multi_batch=self.multi_batch).items

                # 因为可能存在 bias, 所以输入必须要先还原成浮点数, 再利用 fc 计算
                in_ = in_ / in_scale
            ...
            # step 1. 将输入数据转成 half_level 范围以内的定点数
            # if not self.quant_all:
            #     in_ = x.data
            #     in_scale = x.scale    
            # else:
            ...
            for k,v in layer_info.c200_mapping_info.mappings.items():
                ...
                # split_config['array_idx'] = int(v.device.split(":")[-1])
                ...
                # split_config['ip'] = device_info[v.device.split(".")[0]].ip
            ...
            # step3. 计算
            # calc
            output, output_scale, maxp, minp, shift_scale = calc_mvm(weight_addr_list,in_,in_scale,[row_repeat_num,col_repeat_num],
                                    out_channel,output_half_level,weight_scale,dac_bits,adc_bits,self.device_adc_scale_data,
                                    reg_shift_mode,shift_expansion_mode,self.mapped_array_data,assigned_output_quant_scale,
                                    output_quant_mode, it_time, bias_digital=bias_digital, bias=bias, n_scale=n_scale, adc_clamp = adc_clamp, ADC_LUT = ADC_LUT, 
                                    adc_quant = adc_quant,macro_method=self.macro_method, runtime = runtime,
                                    multi_batch=self.multi_batch, fit_k = fit_k, fit_bias = fit_bias)
            ...
            if self.multi_batch:
                ...
                # recover the dimension of shift_scale when the scale is not specified
                if output_quant_mode == 1:
                    shift_scale = np.squeeze(shift_scale, axis=1)
                
                # batch
                batch = in_.shape[1]
                # 计算每个batch的overshoot比例
                maxp = np.array(maxp).sum()
                minp = np.array(minp).sum()
                
                # 计算所有batch的overshoot平均比例
                maxp = maxp / batch
                minp = minp / batch
            
            return CNT.to_cimtensor(data=output, scale=output_scale,
                                    max_percent=maxp, min_percent=minp,
    ...
    def fn_conv2d(self, x, **kwargs):
        ...
        input:
          x 输入为 CIMTensor类型数据 
          输入 data 有可能是half level范围内的定点数 --> quant_all = True
          也有可能是浮点数 --> quant_all == False
        weight / bias:
          权重和bias数据也是 CIMTensor类型数据，同时数值大小是 half level 范围内的定点数，(外部量化完成)
        =================================================
        '''
        ...
        if 'device_info' not in kwargs.keys():
            if not self.quant_all:
                ...
                in_, in_scale = CNT.to_cimtensor_quant(data=in_d.data, half_level=self.half_level,
                                                       method=self.chip_quant_method,thr=self.thr,
                                                       multi_batch=self.multi_batch).items
                # 因为可能存在 bias, 所以输入必须要先还原成浮点数, 再利用 conv_2d 计算
                in_ = in_ / in_scale
                in_scale = np.ones_like(in_scale)
            ...
            # 重排 
            if self.weight_format == 'CHW':
                array_input = feature_map_to_input(in_, stride = stride, kernel_size = kernel_size,
                                            padding = padding, multi_batch = self.multi_batch)
            elif self.weight_format == 'HWC':
                # 此时输入应该默认为HWC的格式
                array_input = feature_map_to_input_np_HWC(in_, stride = stride, kernel_size = kernel_size,
                                            padding = padding, multi_batch = self.multi_batch)
            else:
                raise ValueError(f"暂不支持权重格式{self.weight_format}!!!")
            
            # time2 = time.time()
            # print(f'输入重排 time: {time2 - time1} s')
            # bias
            bias_digital = False
            bias = None
            ...
            # calc
            output_, output_scale, maxp, minp, shift_scale = calc_mvm(weight_addr_list,array_input,in_scale,[row_repeat_num,col_repeat_num],
                                    out_channel,output_half_level,weight_scale,dac_bits,adc_bits,self.device_adc_scale_data,
                                    reg_shift_mode,shift_expansion_mode,self.mapped_array_data,assigned_output_quant_scale,
                                    output_quant_mode, it_time, bias_digital=bias_digital, bias=bias, n_scale=n_scale, adc_clamp = adc_clamp, ADC_LUT = ADC_LUT, 
                                    adc_quant = adc_quant,macro_method=self.macro_method, runtime = runtime,
            ...
            # 此时输出默认为CHW排列
            # 如果是weight_format是HWC，则输出也进行相应变换
            # 变换之后方便之后的全连接层处理
    ...
    def fn_conv_transpose2d(self, x, **kwargs):
        ...
        if 'device_info' not in kwargs.keys():
            if not self.quant_all:
                ...
                # 因为可能存在 bias, 所以输入必须要先还原成浮点数, 再利用 conv_transpose2d 计算
                in_ = in_ / in_scale
                in_scale = np.ones_like(in_scale)
    ...
    def fn_concat(self, *x, axis):
        ...
        # re = self._be.concatenate([CNT.scale_recover(cnt).data for cnt in x],axis=axis)
        data = []
        for cnt in x:
    ...
    def fn_selu(self, x, *, alpha, gamma):
        data = CNT.scale_recover(x).data
        re = self._be.where(data < 0, (self._be.exp(data) - 1) * alpha * gamma,
                              data * gamma)
        if self.quant_all:
            return CNT.to_cimtensor_quant(data=re,half_level=self.half_level,multi_batch=self.multi_batch)
        else:
    ...
    def fn_avg_pool1d(self, x, **kwargs):
        ...
        data = CNT.scale_recover(x).data
        # re = self._pool(data, ndim=1, func=self._be.mean, **kwargs)
    ...
    def fn_avg_pool2d(self, x, **kwargs):
        data = CNT.scale_recover(x).data
        # re = self._pool(data, ndim=2, func=self._be.mean, **kwargs)
    ...
    def fn_avg_pool3d(self, x, **kwargs):
        ...
        data = CNT.scale_recover(x).data
        # re = self._pool(data, ndim=3, func=self._be.mean, **kwargs)
    ...
    def fn_max_pool1d(self, x, **kwargs):
        ...
        data = CNT.scale_recover(x).data
        # re = self._pool(data, ndim=1, func=self._be.amax, dont_pad=True,
        #                   **kwargs)
    ...
    def fn_max_pool2d(self, x, **kwargs):
        ...
        data = CNT.scale_recover(x).data
        # re = self._pool(data, ndim=2, func=self._be.amax, dont_pad=True,
        #                   **kwargs)
    ...
    def fn_max_pool3d(self, x, **kwargs):
        ...
        data = CNT.scale_recover(x).data
        # re = self._pool(data, ndim=3, func=self._be.amax, dont_pad=True,
        #                   **kwargs)
    ...
    def fn_add(self, *args):
        assert len(args) >= 2
        re = CNT.scale_recover(args[0]).data
        for i in range(1, len(args)):
            if str(type(args[i])) == "<class 'numpy.ndarray'>":
                re += args[i]
            else:
                re += CNT.scale_recover(args[i]).data
        # data = CNT.scale_recover(x).data
