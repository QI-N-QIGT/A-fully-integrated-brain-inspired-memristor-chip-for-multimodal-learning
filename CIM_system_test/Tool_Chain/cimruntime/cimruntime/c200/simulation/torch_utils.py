...
def get_bit_value(tensor, bit_index, device='cpu'):
    # 确保输入是int8类型
    ...
    # 如果符号位是0，则返回特定位的值
    result = torch.where(sign_bit & (bit_value == 1), torch.tensor(-1, dtype=torch.int8).to(device), bit_value)
...
def C200_MVM_1bIN_4bOUT_SIM(input_data, weight_data, *, DAC_noise = 0, conductance_noise = 0.0, ADC_noise = 0, ADC_offset = 0,
    ...
    # output_quant = output_current / current_range[f'{integration_time*100}'] * 127 #  8-bit quantization
    output_quant = torch.round(output_quant).to(torch.int32)
    
    # Tuning
    # scale_ = torch.max(output_quant).cpu().detach().numpy() / 7
    # if torch.max(output_current) > 130:
    #     print(f'Max Output Current: {torch.max(output_current)}')
    # print(f'Current integration time: {integration_time}')
    # print(f'Current quantization range: {current_range[f"{integration_time*100}"]}')
    # scale_ = torch.max(output_quant).cpu().detach().numpy() / 127
    
    # current_quantization_range = current_range[f'{integration_time*100}']
    # diff = []
    # target = current_quantization_range * scale_
    # for k,v in current_range.items():
    #     diff.append(abs(v-target))
    
    # #     
    # min_index = np.argmin(np.array(diff))
    # print(f'Best it time: {min_index + 1} !!!')
    # # print(f'Best it time: {min_index + 1}, best quantization range: {current_range[f"{(min_index+1)*100}"]}, diff: {diff[min_index]} !!!')
    # # second quantization
    # # output_quant = output_current / current_range[f'{(min_index+1)*100}'] * 7 # 4-bit quantization
    # output_quant = output_current / current_range[f'{(min_index+1)*100}'] * 127 #  8-bit quantization
    ...
    # print(output_quant.max())
    # print(output_quant.min())
    # input()
    # 量化偏移误差
    if ADC_offset != 0:
    ...
    # output_quant = torch.clamp(output_quant, min=-128, max=127) # 8-bit quantization
    
    # output_quant = output_current
    # output_quant = output_quant * scale_ # 4-bit quantization
    # output_quant = output_quant * scale_ * 7 / 127 # 8-bit quantization
...
def C200_Conv2d_SIM(inputs, weights, *, kernel = 1, stride = 1, padding = 0, input_quant_scale = 1.0, output_dequant_scale = 1.0,
    ...
    if input_expansion_method == 0:
        ...
        outputs = 0
        for i in range(len_):
            partial_sum = C200_MVM_1bIN_4bOUT_SIM(inputs_int[:,:,i], weights, conductance_noise=PE_weight_noise, ADC_offset = ADC_offset, integration_time=integration_time, device=device)
        ...
        # # 按bit计算三次
        # output_x4 =  C200_MVM_1bIN_4bOUT_SIM(input_x4, weights, conductance_noise=PE_weight_noise, ADC_offset = ADC_offset, integration_time=integration_time, device=device)
        # output_x2 =  C200_MVM_1bIN_4bOUT_SIM(input_x2, weights, conductance_noise=PE_weight_noise, ADC_offset = ADC_offset, integration_time=integration_time, device=device)
        # output_x1 =  C200_MVM_1bIN_4bOUT_SIM(input_x1, weights, conductance_noise=PE_weight_noise, ADC_offset = ADC_offset, integration_time=integration_time, device=device)
        ...
        for bit_index in range(bit_length):
            input_bit_value = get_bit_value(inputs_int, bit_index, device=device)
...
def C200_FC_SIM(inputs, weights, *, input_quant_scale = 1, output_dequant_scale = 1.0, activation_bits = 4, 
    ...
    if input_expansion_method == 0:
        ...
        outputs = 0
        for i in range(len_):
            partial_sum = C200_MVM_1bIN_4bOUT_SIM(inputs_int[:,:,i], weights, integration_time=integration_time, ADC_offset = ADC_offset, device=device)
            outputs = outputs + partial_sum
...
def C200_GlobalAvgpool2d(input_data, out_size=1, device='cpu'):
