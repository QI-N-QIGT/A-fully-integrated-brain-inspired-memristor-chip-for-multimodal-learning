...
class CIMNumpyTensor:
    '''
    创建一个面向模拟CIM运算的数据类型
    该类型主要将CIM计算时特有属性scale,overshoot_percent等参数与numpy.ndarray对象封装在一起
    max_percent: 模拟计算等于ADC最大量化值的比例
    min_percent: 模拟计算等于ADC最小量化值的比例
    shift_scale: 模拟计算当前输出与下一层输入的比例系数
    =====>
    Note:max_percent与min_percent参数只代表CIM计算之后的CNT数据特性，
        当CNT做完运算之后，则该参数自动变为0，将不具有实际意义；
        shift_scale：观测系数，CNT做运算之前可观测的值，CNT做完运算之后，该值也不具有实际意义。
    '''
