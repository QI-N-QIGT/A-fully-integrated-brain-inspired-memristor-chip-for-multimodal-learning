...
g_lidar_subscriber = None
g_speed_calc_local = False 
    ...
    if cmd == "0F":  # 进入界面
        ...
        if speed_x == 0 and speed_y == 0:
            ...
            if g_car_type == g_bot.CARTYPE_R2:
                ...
                g_bot.set_car_motion(speed_x*1.8/10, speed_y/2, 0)
                publish_car_speed(speed_x*1.8/10, speed_y/2)
            else:
                g_bot.set_car_motion(speed_x, speed_y, 0)
        if g_debug:
            print("speed_x:%.2f, speed_y:%.2f" % (speed_x/10, speed_y/2))
...
def init_tcp_socket():
    ...
    task_tcp = threading.Thread(target=start_tcp_server, name="task_tcp", args=('192.168.1.11', 6000))
...
def get_radar_speed(msg, threshold = 1.2):
    dis = np.array(msg.ranges)
    index = np.array(msg.intensities) == 0
    dis[index] = np.max(dis[(1 - index).astype(np.bool8)])
    # 3 labels straight left and right
#    default straight
#    obstcal in forward and right turn left
#    obstcal in left turn right
...
def get_radar_speed_2(msg, threshold=0.8):
    dis = np.array(msg.ranges)
    index = np.array(msg.intensities) == 0
    dis[index] = np.max(dis[(1 - index).astype(np.bool8)])
    # 3 labels straight left and right
    ...
    # obstcal in forward and right turn left
    # obstcal in left turn right
    dis_frd_right = dis[:336]
    dis_left = dis[337:]

    x, y = 0.2, 0
    if min(dis_frd_right) < threshold and min(dis_left) > threshold:
        x, y = 0.05, 0.02
    elif min(dis_left) < threshold and min(dis_frd_right)>threshold:
        x, y = 0.05, -0.02
    elif min(dis_left) < threshold and min(dis_frd_right)<threshold:
        x,y = -0.1, 0
...
def lidar_callback(msg):
    speed_x, speed_y = get_radar_speed(msg)
    g_bot.set_car_motion(speed_x, speed_y, 0)
    publish_car_speed(speed_x, speed_y)
    
def speed_callback(msg):
    speed_x, speed_y = msg.data
    #print("speed_x=%f, speed_y=%f" %(speed_x, speed_y))
    g_bot.set_car_motion(speed_x, speed_y, 0)

def thread_car_movement_control():
    rospy.spin()
...
def sigint_handler(signal, frame):
    ...
    rospy.loginfo(f"Shutting down the node... {signal}")
    print(signal)
    print(frame)
...
if __name__ == '__main__':
    ...
    g_speed_publisher = rospy.Publisher('car_speed', Float32MultiArray, queue_size=10)
    if g_speed_calc_local:
        g_lidar_subscriber = rospy.Subscriber('/scan', LaserScan, lidar_callback)
    else:
        g_speed_subscriber = rospy.Subscriber('/a111_car_speed',  Float32MultiArray, speed_callback)

    task_car_movement = threading.Thread(target=thread_car_movement_control, name="task_car_movement")
    task_car_movement.setDaemon(True)
    task_car_movement.start()
